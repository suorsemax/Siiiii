...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@YFYYF"))
...
@sython.on(events.NewMessage(outgoing=False, pattern='max'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply('Hello Im Max  @eeeceeee')
