...
async def _(event):
    if ispay2[0] == "yes":
        ...
        choice = str(msg[1])
        trys = 0
        await event.edit(f"Okay I'll switch to installing the ID `{username}` on `{ch}` , By number `{msg[0]}` Of attempts !")
        ...
        for i in range(int(msg[0])):
            ...
            if "Available" in isav:
                ...
                try:
                    ...
                    await event.client.send_message(event.chat_id, f'''𓆩 We are the strongest @eeeceeee !'
⎱UserName: ❲ @{username} ❳
⎱Save: ❲ channel ❳''')
                    break
                    ...
                    await sython.send_message(event.chat_id, f'''Error with error {username}
  error :
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        break
                    else:
                        await sython.send_message(event.chat.id, " ID available !")
            else:
                pass
            trys += 1
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "!Installation finished")
...
⎱Save: ❲ channel ❳''')
                        ...
                        await event.client.send_message(event.chat_id, f"Forbidden {username} ❌❌")
                        break
                    except Exception as eee:
   ...
   error:
    {str(eee)}''')
        ...
        if msg[0] == "يدوي":  # تثبيت يدوي يوزر قناة
            await event.edit(f"Okay I'll switch to installing the ID `{username}` on `{ch}` !")
            msg = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
            username = str(msg[0])
            ch = str(msg[1])
            try:
